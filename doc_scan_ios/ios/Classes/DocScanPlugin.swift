...
public class DocScanPlugin: NSObject, FlutterPlugin, VNDocumentCameraViewControllerDelegate {
    ...
    public func documentCameraViewController(_ controller: VNDocumentCameraViewController, didFinishWith scan: VNDocumentCameraScan) {
        ...
        if selectedFormat == "jpeg" {
            // For JPEG format, save each image separately as before
            ...
            // For PDF format, create a single PDF with all images
            let images = (0..<scan.pageCount).map { scan.imageOfPage(at: $0) }
            let pdfPath = saveImagesAsPDF(images: images)
    ...
    /// Helper function to save multiple images as a single PDF
    private func saveImagesAsPDF(images: [UIImage]) -> String {
        let pdfPath = NSTemporaryDirectory().appending("\(UUID().uuidString).pdf")
        let pdfData = NSMutableData()
        UIGraphicsBeginPDFContextToData(pdfData, CGRect(origin: .zero, size: images.first!.size), nil)
        for image in images {
